...
import { Visibility, CloudDownloadRounded, LinkedIn, GitHub, EmailRounded } from '@mui/icons-material'
    ...
    return (
        <Stack direction='column' gap={2} sx={{ justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Stack direction='row' gap={3} sx={{ marginTop: '2rem' }}>
                <Button variant="contained" size='large' endIcon={<Visibility />} onClick={() => navigate('/portfolio')}>My Work</Button>
                <Button variant="outlined" size='large' endIcon={<CloudDownloadRounded />} onClick={() => window.open('https://1drv.ms/b/s!AqUO-r5Zn68aieYq4jZ_LEf9aDFv4A', '_blank')}>Resume</Button>
            </Stack>
            <Stack direction='column' gap={2} sx={{ position: 'absolute', top: '40%', right: '5%', display: { xs: 'none', md: 'flex' } }}>
                <Fab color='primary' onClick={() => window.open('https://www.linkedin.com/in/kotagiri-vinod-a10ba856/', '_blank')}>
                    <LinkedIn />
                </Fab>
                <Fab color='primary' onClick={() => window.open('https://www.github.com/vinodkotagiri', '_blank')}>
                    <GitHub />
                </Fab>
